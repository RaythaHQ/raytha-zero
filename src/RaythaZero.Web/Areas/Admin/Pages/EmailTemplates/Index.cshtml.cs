...
public class Index : BaseAdminPageModel, IHasListView<Index.EmailTemplatesListItemViewModel>
{
    ...
    public async Task<IActionResult> OnGet(
        string search = "", string orderBy = $"Subject {SortOrder.ASCENDING}", int pageNumber = 1, int pageSize = 50)
    {
        ...
        ListView = new ListViewModel<EmailTemplatesListItemViewModel>(items, response.Result.TotalCount);
        return Page();
    }
