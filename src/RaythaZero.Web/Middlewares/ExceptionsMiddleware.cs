    ...
    public static Action<IApplicationBuilder> ErrorHandler(string pathBase)
    {
        return errorApp =>
        {
            errorApp.Run(async context =>
            {
                ...
                byte[] errorBytes;
                if (path.Value.ToLower().StartsWith($"{pathBase}/admin/api"))
                {
                ...
                else
                {
                    if (error.Error is NotFoundException)
                    {
                        context.Response.Redirect($"{pathBase}/admin/404");
                    }
                    else if (error.Error is FormatException)
                    {
                        context.Response.Redirect($"{pathBase}/admin/404");
                    ...
                    {
                        context.Response.Redirect($"{pathBase}/admin/403");
                    }
                    else
                    {
                        context.Response.Redirect($"{pathBase}/admin/500");
                    }
                }
            });
        };
